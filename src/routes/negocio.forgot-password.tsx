...
function BusinessForgotPassword() {
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!email) throw new Error("Ingresa el correo del responsable");

      const redirectTo =
        typeof window !== "undefined"
          ? `${window.location.origin}/negocio/forgot-password?reset=1`
          : "http://localhost:3000/negocio/forgot-password?reset=1";
      ...
      <main className="min-h-dvh bg-background">
        <header className="flex items-center justify-between px-4 py-4 md:px-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="ml-1 rounded-md bg-surface-2 px-2 py-0.5 text-[10px] uppercase text-muted-foreground">
              B2B
            </span>
          </div>
        ...
        <div className="mx-auto flex w-full max-w-md flex-col px-4 pb-10 pt-2 md:pt-8">
          <div className="md:rounded-2xl md:border md:border-border md:bg-surface md:p-8">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              {resetMode
                ? "Elige una contraseña nueva para el acceso del negocio."
                : "Te enviaremos un enlace al correo del responsable del negocio."}
            </p>
            ...
            {resetDone ? (
              <div className="mt-6 space-y-4">
                ...
                <Link to="/negocio/login" className="btn-primary w-full">
                  Iniciar sesión
                </Link>
              </div>
            ) : !exchangeResolved ? (
              ...
              <form onSubmit={onSubmit} className="mt-6 space-y-5">
                <div>
                  <label className="label-field">Correo del responsable</label>
                  <input
                    name="email"
                    type="email"
                    required
            ...
            <p className="mt-6 text-center text-xs text-muted-foreground">
              <Link to="/negocio/login" className="text-primary hover:underline">
